...
import { Plug, Code, Server, Webhook, KeyRound } from 'lucide-react';
...
export function ConnectPage() {
    return (<PageContainer size="wide">
      <PageHeader title="Connect" description="API reference, authentication, and integration examples" icon={<Plug className="size-5"/>}/>
      ...
      <Card padding="none" className="mt-3">
        <Tabs defaultValue="openai">
          <div className="px-4 pt-4">
            <TabsList>
              {Object.entries(EXAMPLES).map(([k, v]) => (<TabsTrigger key={k} value={k}>{v.title}</TabsTrigger>))}
            </TabsList>
          </div>
          {Object.entries(EXAMPLES).map(([k, v]) => (<TabsContent key={k} value={k} className="px-4 pb-4 mt-2">
      ...
      <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        ].map((s) => (<div key={s.name} className="flex items-center gap-3 rounded-lg border border-border bg-surface-2 p-2.5">
        ...
        ].map((a) => (<div key={a.name} className="flex items-center gap-2 rounded-md border border-border bg-surface-2 px-2.5 py-1.5">
