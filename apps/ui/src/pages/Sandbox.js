...
export function SandboxPage() {
    const jobs = useApiData(() => Admin.listSandboxJobs(), [], { refetchInterval: 5000 });
    return (<PageContainer>
      <PageHeader title="Sandbox" description="Ephemeral execution environment for tool and code testing" icon={<Terminal className="size-5"/>} actions={<Button size="sm">
  ...
  "timeoutMs": 5000
      ...
      <div className="mt-3">
        <Card padding="none">
          ...
          {jobs.isLoading ? (<div className="p-3 flex flex-col gap-1.5">
              {Array.from({ length: 3 }).map((_, i) => (<Skeleton key={i} className="h-12 w-full"/>))}
            </div>) : jobs.data && jobs.data.length > 0 ? (<div className="p-1">
              {jobs.data.map((j) => (<div key={j.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-surface-2">
                  ...
                  <StatusPill status={j.status === 'completed' ? 'healthy' :
                    j.status === 'failed' ? 'offline' :
                        j.status === 'running' ? 'pending' : 'unknown'} label={j.status} size="sm" showDot={false}/>
                </div>))}
            </div>) : (<EmptyState title="No jobs" description="Submit a job to test code in a sandboxed environment."/>)}
        </Card>
      </div>
    </PageContainer>);
}
