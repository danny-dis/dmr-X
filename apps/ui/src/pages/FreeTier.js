...
function isFreeProvider(entry) {
    if (!entry.models || entry.models.length === 0)
        return false;
    return entry.models.some((m) => m.freeTier != null);
}
export function FreeTierPage() {
    ...
    const catalog = useApiData(() => Admin.getCatalog(), [], { refetchInterval: 60000 });
    ...
    const filtered = freeCatalog.filter((entry) => {
        if (debounced) {
            ...
            if (!haystack.includes(debounced.toLowerCase()))
                return false;
        }
        if (category === 'all')
            return true;
        if (category === 'cloud')
            return cloudCategories.has(entry.category);
        if (category === 'local')
            return entry.category === 'local';
        if (category === 'hosting')
            return entry.category === 'hosting';
        return true;
    });
    const avgRpm = (() => {
        const rpms = [];
        for (const e of freeCatalog) {
            for (const m of e.models ?? []) {
                const ft = m.freeTier;
                if (ft?.rateLimits?.rpm != null)
                    rpms.push(ft.rateLimits.rpm);
            }
        }
        if (rpms.length === 0)
            return '—';
    ...
    return (<PageContainer size="wide">
      <PageHeader title="Free Tier Providers" description="Connect to free-tier providers with no credit card required" icon={<Gift className="size-5"/>} actions={<>
      ...
      <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatTile label="Available" value={freeCatalog.length} icon={<Globe className="size-3.5"/>} hint="free providers in catalog"/>
        <StatTile label="Connected" value={connectedFree.length} icon={<Server className="size-3.5"/>} tone="success" hint="active free providers"/>
        <StatTile label="Catalog Models" value={formatNumber(totalFreeModels)} icon={<Zap className="size-3.5"/>} tone="primary" hint="free-tier models available"/>
        <StatTile label="Avg. RPM" value={avgRpm} icon={<RefreshCw className="size-3.5"/>} tone="accent" hint="average rate limit"/>
      </div>
      ...
      <div className="mt-5">
        <Card padding="md">
          ...
          {providers.isLoading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {Array.from({ length: 3 }).map((_, i) => (<Skeleton key={i} className="h-28 w-full"/>))}
            </div>) : connectedFree.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {connectedFree.map((p) => (<FreeProviderCard key={p.id} provider={p} isFavorite={favorites.includes(p.id)} onToggleFavorite={toggleFav} onTest={onTest} onSelect={(prov) => {
            ...
            </div>) : (<div className="text-center py-8 text-xs text-fg-subtle">
        ...
        <Card padding="md">
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="flex-1 max-w-md">
              <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search free providers…" prefix={<Search className="size-3.5"/>}/>
            </div>
            <div className="flex items-center gap-1">
              {['all', 'cloud', 'local', 'hosting'].map((c) => (<button key={c} onClick={() => setCategory(c)} className={cn('h-9 px-3 rounded-lg text-xs font-medium transition-colors', category === c
          ...
          {catalog.isLoading ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {Array.from({ length: 6 }).map((_, i) => (<Skeleton key={i} className="h-32 w-full"/>))}
            </div>) : filtered.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filtered.map((entry) => {
                const isConnected = connectedIds.has(entry.id);
                const freeModels = (entry.models ?? []).filter((m) => m.freeTier != null);
                const anyRpm = freeModels.length > 0
                    ? freeModels[0].freeTier.rateLimits.rpm
                    : 0;
                return (<Card key={entry.id} padding="md" interactive={!isConnected} className={cn('group flex flex-col', isConnected && 'opacity-75')} onClick={() => {
                        if (!isConnected) {
                            setSelectedTemplate(entry);
                            setAddDialogOpen(true);
                        }
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {isConnected ? (<Badge tone="success" size="sm">Connected</Badge>) : (<Badge tone="muted" size="sm" icon={<Gift className="size-2.5"/>}>
                          Free
                        </Badge>)}
                    </div>
                    ...
                    <div className="mt-3 flex items-center gap-3 text-[11px] text-fg-subtle">
                      ...
                      {anyRpm > 0 && (<span className="flex items-center gap-1">
                          <RefreshCw className="size-3"/>
                          {formatNumber(anyRpm)} RPM
                        </span>)}
                      {entry.signupUrl && (<span className="flex items-center gap-1 ml-auto">
                          <ExternalLink className="size-3"/>
                          <a href={entry.signupUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline" onClick={(e) => e.stopPropagation()}>
                            Get Key
                          </a>
                        </span>)}
                    </div>

                    {isConnected && (<div className="mt-3 pt-2 border-t border-border/60 flex items-center gap-2">
                        <Badge tone="muted" size="sm" icon={<KeyRound className="size-2.5"/>}>
                          {entry.authMethod ?? 'api_key'}
                        </Badge>
                        {entry.category && (<Badge tone="muted" size="sm">{entry.category}</Badge>)}
                      </div>)}
                  </Card>);
            })}
            </div>) : (<Card padding="none" className="border-dashed">
              <EmptyState title={query ? 'No providers match' : 'No free providers found'} description={query
      ...
      <FreeTierDrawer provider={selectedProvider} open={selectedProvider !== null} onOpenChange={(open) => {
            if (!open)
                setSelectedProvider(null);
        }} onCreated={() => void providers.refetch()}/>

      <AddProviderDialog open={addDialogOpen} onOpenChange={setAddDialogOpen} template={selectedTemplate} onCreated={() => void providers.refetch()}/>
    </PageContainer>);
}
