      ...
      {status.isLoading ? (
          ...
          <CardContent className="flex items-start gap-3 pt-5">
            ...
            <div className="text-sm">
              <div className="font-medium text-fg">
                {enabled ? 'A2A is enabled but not answering' : 'A2A is disabled'}
              </div>
              {/* The backend returns the exact fix; show it verbatim rather
                  than paraphrasing into something vaguer. */}
              <p className="mt-1 leading-relaxed text-fg-muted">{status.data?.hint}</p>
              {status.data?.error && (
                <p className="mt-1 font-mono text-2xs text-fg-subtle">{status.data.error}</p>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-2">
          <Card className="p-4">
            ...
            <Code copyable className="mt-1.5">{status.data!.agentCardUrl!}</Code>
          </Card>
          <Card className="p-4">
            <div className="text-2xs uppercase tracking-wide text-fg-subtle">
              JSON-RPC endpoint
              {status.data?.protocolVersion && (
                <Badge tone="info" variant="soft" size="sm" className="ml-2">
                  v{String(status.data.protocolVersion)}
                </Badge>
              )}
            </div>
            <Code copyable className="mt-1.5">{status.data!.jsonRpcUrl!}</Code>
          </Card>
        </div>
      )}
...
function AgentCardPanel({ enabled }: { enabled: boolean }) {
  const { data, isLoading } = useAgentCard(enabled);
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Detail label="URL" value={data?.url ?? '—'} mono />
          <Detail label="Protocol" value={data?.protocolVersion ?? '—'} />
          <Detail label="Preferred transport" value={data?.preferredTransport ?? '—'} />
...
function TasksPanel({ enabled }: { enabled: boolean }) {
  ...
  const { data, isLoading } = useA2ATasks(state, enabled);
  ...
  if (!enabled) {
    return (
      <Card className="mt-4">
        <CardContent className="pt-6">
          <EmptyState icon={<Network className="size-6" />} title="A2A is not running" />
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="mt-4">
      <CardHeader className="flex-row items-center justify-between">
        ...
        <div className="flex gap-1">
          <Button size="sm" variant={!state ? 'secondary' : 'ghost'} onClick={() => setState(undefined)}>
            All
          </Button>
          {(['working', 'completed', 'failed'] as A2ATaskState[]).map((s) => (
            <Button key={s} size="sm" variant={state === s ? 'secondary' : 'ghost'} onClick={() => setState(s)}>
              {s}
            </Button>
          ))}
        </div>
      ...
      <CardContent>
        {isLoading ? (
          ...
          <p className="py-8 text-center text-sm text-fg-muted">
            No tasks. Tasks appear here when a peer agent sends this instance work.
          </p>
        ) : (
