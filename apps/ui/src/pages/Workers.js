...
export function WorkersPage() {
    const workers = useApiData(() => Admin.listWorkers(), [], { refetchInterval: 5000 });
    return (<PageContainer>
      <PageHeader title="Workers" description="Background job workers — quota, telemetry, billing, garbage collection" icon={<Cpu className="size-5"/>} actions={<Button size="sm">
            <Plus className="size-3"/>
            Register worker
          </Button>}/>

      <div className="mt-5">
        {workers.isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Array.from({ length: 4 }).map((_, i) => (<Skeleton key={i} className="h-32 w-full"/>))}
          </div>) : workers.data && workers.data.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {workers.data.map((w) => (<Card key={w.id} padding="md">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <StatusPill status={w.draining ? 'warning' : w.alive ? 'online' : 'offline'} label={w.draining ? 'Draining' : w.alive ? 'Active' : 'Down'} size="sm" showDot={!w.draining && w.alive}/>
                </div>
                ...
                {w.load != null && (<div className="mt-3">
                ...
                <div className="mt-3 flex items-center gap-1.5 justify-end">
                  {w.draining ? (<Button size="sm" variant="secondary">
            ...
            <EmptyState title="No workers running" description="Workers are spawned automatically by the gateway as needed."/>
          </Card>)}
      </div>
    </PageContainer>);
}
