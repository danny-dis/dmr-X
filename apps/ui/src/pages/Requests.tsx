...
export function RequestsPage() {
  ...
  const filtered = all.filter((e) => {
    ...
    if (statusFilter === 'ok' && e.status === 'error') return false;
  ...
  return (
    <PageContainer size="wide">
      <PageHeader
        title="Requests"
        description="Live stream of all requests routed through the gateway"
      ...
      <div className="mt-5 grid grid-cols-2 md:grid-cols-5 gap-2">
        {['request', 'routing', 'model', 'tool', 'system'].map((k) => (
          <button
            key={k}
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-border">
          <div className="flex-1 max-w-sm">
            <Input
              ...
              placeholder="Search by tenant, model, message…"
