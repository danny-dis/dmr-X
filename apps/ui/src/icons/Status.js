...
export function HealthDot({ size = 8, className = '', color = 'currentColor' }) {
    return (<span className={`relative inline-flex ${className}`} style={{ width: size, height: size }}>
      <span className="absolute inset-0 rounded-full" style={{ background: color }}/>
      <span className="absolute inset-0 rounded-full animate-ping" style={{ background: color, opacity: 0.5, animationDuration: '2s' }}/>
    </span>);
}
export function PulseRing({ size = 24, className = '' }) {
    return (<span className={`relative inline-flex items-center justify-center ${className}`} style={{ width: size, height: size }}>
      <span className="absolute inset-0 rounded-full" style={{ background: 'currentColor', opacity: 0.15 }}/>
      <span className="absolute rounded-full pulse-dot" style={{
            inset: 0,
...
export function Sparkline({ points, width = 80, height = 24, stroke = 'currentColor', fill = 'transparent', strokeWidth = 1.5 }) {
    if (points.length < 2)
        return null;
...
export function ConnectionLine({ orientation = 'horizontal', className = '' }) {
    if (orientation === 'curved') {
        return (<svg width="40" height="20" viewBox="0 0 40 20" className={className}>
        <path d="M0 10 Q 20 0, 40 10" fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 3"/>
      </svg>);
    }
    return (<svg width={orientation === 'vertical' ? 4 : 40} height={orientation === 'vertical' ? 40 : 4} className={className}>
...
export function RingLoader({ size = 24, className = '' }) {
    return (<svg width={size} height={size} viewBox="0 0 24 24" className={className}>
