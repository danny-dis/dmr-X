...
export function DonutChart({ data, size = 200, thickness = 22, showLegend = false, showLabels = false, centerLabel, centerValue, className, }) {
    ...
    if (total === 0) {
        ...
        <div className="rounded-full border-2 border-dashed border-border-strong" style={{ width: size - 8, height: size - 8 }}/>
      </div>);
    }
    let acc = 0;
    ...
    return (<div className={cn('flex items-center gap-4', className)}>
        ...
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          {segments.map((s, i) => {
            if (s.end - s.start >= Math.PI * 2 - 0.001) {
                return (<circle key={i} cx={cx} cy={cy} r={radius - thickness / 2} fill="none" stroke={s.color} strokeWidth={thickness}/>);
            }
            ...
            return (<path key={i} d={`M ${x1} ${y1} A ${radius - thickness / 2} ${radius - thickness / 2} 0 ${large} 1 ${x2} ${y2}`} fill="none" stroke={s.color} strokeWidth={thickness} strokeLinecap="round"/>);
        })}
        </svg>
        {(centerLabel || centerValue) && (<div className="absolute inset-0 flex flex-col items-center justify-center text-center">
      ...
      {showLegend && (<div className="flex flex-col gap-1.5 min-w-0">
          {data.map((d) => (<div key={d.label} className="flex items-center gap-2 text-xs">
              ...
              {showLabels && (<span className="text-fg-subtle tabular-nums w-10 text-right">
...
export function Sunburst({ data, size = 280, className }) {
    return (<div className={cn('flex justify-center', className)}>
      <SunburstNode data={data} size={size} depth={0} index={0}/>
    </div>);
}
function SunburstNode({ data, size, depth, index, parentColor, }) {
    const total = data.reduce((s, d) => s + d.value, 0);
    if (total === 0)
        return null;
    const cx = size / 2;
    ...
    return (<g>
      {arcs.map((a) => {
            ...
            return (<g key={a.label + depth}>
            <path d={`M ${x1} ${y1} L ${x2} ${y2} A ${outerR} ${outerR} 0 ${large} 1 ${x3} ${y3} L ${x4} ${y4} A ${innerR} ${innerR} 0 ${large} 0 ${x1} ${y1} Z`} fill={a.color} fillOpacity={depth === 0 ? 0.85 : 0.55 - depth * 0.1} stroke="var(--bg)" strokeWidth={1.5}>
              <title>{a.label} · {a.value}</title>
            </path>
            {a.children && depth < 2 && (<SunburstNode data={a.children} size={size} depth={depth + 1} index={a.i} parentColor={a.color}/>)}
          </g>);
        })}
    </g>);
}
