...
export interface HeatmapProps {
  ...
  xLabels: string[];
  yLabels: string[];
  colorScale?: { low: string; high: string; empty?: string };
...
export function Heatmap({
  ...
  xLabels,
  yLabels,
  colorScale = { low: '#1F2230', high: '#7C5CFF', empty: 'transparent' },
      ...
      <svg width={width} height={height} className="font-mono">
        {yLabels.map((y, yi) => (
          <text
            key={`y-${y}`}
            x={labelWidth - 4}
            y={labelHeight + yi * (cellSize + yGap) + cellSize / 2 + 3}
            textAnchor="end"
            fill="var(--text-dim)"
            fontSize={9}
          >
        ...
        {xLabels.map((x, xi) => (
          <text
            key={`x-${x}`}
            x={labelWidth + xi * (cellSize + xGap) + cellSize / 2}
            y={labelHeight - 4}
            textAnchor="middle"
            fill="var(--text-dim)"
            fontSize={9}
            transform={`rotate(-30, ${labelWidth + xi * (cellSize + xGap) + cellSize / 2}, ${labelHeight - 4})`}
          >
            {x}
          </text>
        ))}
