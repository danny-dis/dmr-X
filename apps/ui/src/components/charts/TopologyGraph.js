...
export function TopologyGraph({ nodes, edges, height = 480, onNodeClick, className, }) {
    const width = 800;
    const layout = React.useMemo(() => layoutGraph(nodes, edges, width, height), [nodes, edges, height]);
    return (<div className={cn('relative overflow-auto rounded-xl border border-border bg-surface-1', className)}>
      <svg width={layout.viewWidth} height={height} viewBox={`0 0 ${layout.viewWidth} ${height}`} className="font-sans">
        ...
        {layout.edges.map((e, i) => {
            ...
            return (<g key={i}>
              <path d={path} fill="none" stroke={e.active ? '#7C5CFF' : 'var(--border-2)'} strokeWidth={Math.max(1, e.weight ?? 1)} strokeOpacity={e.active ? 0.9 : 0.6} strokeDasharray={e.active ? '4 4' : undefined} markerEnd="url(#topo-arrow)" className={e.active ? 'animate-[dash_1.6s_linear_infinite]' : ''}>
                {e.label && (<text x={(e.x1 + e.x2) / 2} y={(e.y1 + e.y2) / 2 - 4} textAnchor="middle" fill="var(--text-dim)" fontSize={9}>
                    {e.label}
                  </text>)}
              </path>
            </g>);
        ...
        {layout.nodes.map((n) => {
            ...
            return (<g key={n.id} transform={`translate(${n.x}, ${n.y})`} className={onNodeClick ? 'cursor-pointer' : ''} onClick={() => onNodeClick?.(n)}>
              ...
              {n.status === 'online' && (<circle cx={18} cy={-18} r={4} fill="#34D399" stroke="var(--bg)" strokeWidth={1.5}>
