...
>(({ className, ...props }, ref) => (
  ...
  >
    ...
    {(Array.isArray(props.value) ? props.value : Array.isArray(props.defaultValue) ? props.defaultValue : [0]).map(
      (_, i) => (
        <SliderPrimitive.Thumb
          key={i}
          className={cn(
            'block size-4 rounded-full border border-primary bg-white shadow-sm',
