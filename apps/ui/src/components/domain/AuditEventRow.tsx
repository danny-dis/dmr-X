...
const TONE_FOR: Record<string, 'primary' | 'success' | 'warning' | 'danger' | 'muted'> = {
  create: 'primary',
  update: 'warning',
  delete: 'danger',
  read: 'muted',
...
export function AuditEventRow({ event, onClick, className }: AuditEventRowProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'flex items-center gap-3 px-3 py-2 rounded-md hover:bg-surface-2 transition-colors',
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 text-sm">
          ...
          {event.target && (
            <span className="font-mono text-[11px] text-fg-muted truncate">{event.target}</span>
          )}
        </div>
        {event.detail && (
          <p className="text-[10px] text-fg-subtle truncate">{event.detail}</p>
        )}
      </div>
