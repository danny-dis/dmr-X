...
import { Zap, Globe, ExternalLink, KeyRound, Activity, } from 'lucide-react';
...
export function FreeTierDrawer({ provider, open, onOpenChange, onCreated }) {
    ...
    const freeModels = (provider.models ?? []).filter((m) => m.freeTier != null);
      ...
      <DrawerContent side="right" size="lg">
        <DrawerHeader>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {provider.category && (<Badge tone="muted" size="sm">{provider.category}</Badge>)}
          </div>
        ...
        <DrawerBody className="space-y-5">
          <section>
            ...
            <div className="rounded-lg border border-border bg-surface-2/40 px-3">
              ...
              <MetaRow label="Auth method" value={provider.authMethod ? (<Badge tone="muted" size="sm" icon={<KeyRound className="size-2.5"/>}>
                      {provider.authMethod}
                    </Badge>) : (<Empty />)}/>
              {provider.signupUrl && (<MetaRow label="Signup" value={<a href={provider.signupUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
                      {provider.signupUrl}
                      <ExternalLink className="size-3"/>
                    </a>} mono/>)}
              <MetaRow label="Models" value={<span>
                    {freeModels.length} free / {totalModels} total
                  </span>}/>
            </div>
          </section>

          {freeModels.length > 0 && (<section>
              <h3 className="text-[10px] font-semibold text-fg-subtle uppercase tracking-wider mb-2 flex items-center gap-1.5">
              ...
              <div className="rounded-lg border border-border bg-surface-2/40 px-3">
                ...
                return (<div key={model.id} className="py-2 border-b border-border/60 last:border-b-0">
                      ...
                      <div className="mt-1 flex items-center gap-3 text-[10px] text-fg-subtle">
                        {ft?.rateLimits?.rpm != null && ft.rateLimits.rpm > 0 && (<span>RPM {formatNumber(ft.rateLimits.rpm)}</span>)}
                        {ft?.rateLimits?.rpd != null && ft.rateLimits.rpd > 0 && (<span>RPD {formatNumber(ft.rateLimits.rpd)}</span>)}
                        {ft?.monthlyTokenBudget != null && ft.monthlyTokenBudget > 0 && (<span>Budget {formatNumber(ft.monthlyTokenBudget)} tok</span>)}
                      </div>
                    </div>);
            })}
                {freeModels.length > 5 && (<p className="text-[10px] text-fg-subtle py-2">
          ...
          <section>
            ...
            <div className="rounded-lg border border-border bg-surface-2/40 px-3">
              <MetaRow label="Intelligence" value={<div className="flex items-center gap-2">
        ...
        <DrawerFooter className="justify-between">
          <div className="flex items-center gap-2">
            {provider.signupUrl && (<Button variant="secondary" asChild>
...
//# sourceMappingURL=FreeTierDrawer.js.map