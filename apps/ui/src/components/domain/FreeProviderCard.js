...
export function FreeProviderCard({ provider, isFavorite = false, onToggleFavorite, onSelect, onTest, className, }) {
    const health = provider.health;
    const healthStatus = health?.status ?? provider.enabled ? 'online' : 'offline';
    const latency = health?.latencyMs;
    const config = provider.config;
    const hasSignupUrl = config?.signupUrl != null;
    const monthlyBudget = config?.monthlyTokenBudget ?? 1000000;
    ...
    return (<Card padding="none" interactive className={cn('group flex flex-col', className)} onClick={() => onSelect?.(provider)}>
      ...
      <div className="px-4 pb-3 flex items-center gap-2 text-[11px] flex-wrap">
        ...
        {provider.capabilities?.length ? (<Badge tone="muted" size="sm">
      ...
      <div className="px-4 pb-4 flex items-center justify-between gap-2 border-t border-border pt-3">
        ...
        <div className="flex items-center gap-2">
          {hasSignupUrl && (<Button size="sm" variant="secondary" onClick={(e) => {
                e.stopPropagation();
                if (config?.signupUrl) {
                    window.open(config.signupUrl, '_blank', 'noopener,noreferrer');
                }
            }}>
...
//# sourceMappingURL=FreeProviderCard.js.map