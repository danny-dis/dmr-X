...
export interface GeminiChatTabProps {
  models: { data?: ApiModel[] };
  modelsForTab: ApiModel[];
}

export function GeminiChatTab({ modelsForTab }: GeminiChatTabProps) {
  const [model, setModel] = React.useState('free');
  const [systemInstruction, setSystemInstruction] = React.useState('');
  const [contents, setContents] = React.useState('[{"role": "user", "parts": [{"text": "Hello"}]}]');
  ...
  const onSend = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Invalid JSON in contents field');
      }
      ...
      const res = await fetch('/v1/gemini/generateContent', {
        method: 'POST',
  ...
  const onReset = () => {
    ...
    setContents('[{"role": "user", "parts": [{"text": "Hello"}]}]');
    setTemperature(0.7);
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Input type="number" min={1} value={maxOutputTokens} onChange={(e) => setMaxOutputTokens(Number(e.target.value) || 1024)} />
            </div>
          </div>
            ...
            <div className="flex items-center gap-3 text-[10px]">
              ...
              {totalTokens > 0 && (
                <span className="text-fg-muted">
                  ...
                  <span className="text-fg font-mono">{totalTokens}</span>
                </span>
              )}
            </div>
          )}
