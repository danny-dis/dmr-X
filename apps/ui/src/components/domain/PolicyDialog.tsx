...
export interface PolicyDialogProps {
  ...
  policy?: ApiPolicyRule | null;
  onSaved?: () => void;
}

const ACTIONS: { id: PolicyAction; label: string; description: string }[] = [
...
export function PolicyDialog({ open, onOpenChange, policy, onSaved }: PolicyDialogProps) {
  ...
  const validate = (): boolean => {
    ...
    if (form.priority) {
      ...
      if (Number.isNaN(n) || n < 0) next.priority = 'Priority must be a non-negative number';
    }
    setErrors(next);
  ...
  const buildMatch = () => {
    const match: ApiPolicyRule['match'] = {};
    if (form.matchModel.trim()) match.model = form.matchModel.trim();
    if (form.matchTenantId.trim()) match.tenantId = form.matchTenantId.trim();
    if (form.matchTag.trim()) match.tag = form.matchTag.trim();
    if (form.matchModality) match.modality = form.matchModality;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const body: Partial<ApiPolicyRule> = {
        id: policy?.id,
        name: form.name.trim(),
          ...
          <DialogBody className="space-y-4">
            <Field>
              ...
              <Input
                ...
                placeholder="Block expensive models for free tier"
                invalid={!!errors.name}
                autoFocus
              />
            ...
            <Field>
              ...
              <Input
                ...
                placeholder="Optional context shown in the policy list"
              />
            </Field>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field>
                  ...
                  <Input
                    ...
                    onChange={(e) => update('matchModel', e.target.value)}
                    placeholder="gpt-4*"
                    mono
                  />
                </Field>
                <Field>
                  ...
                  <Input
                    value={form.matchTenantId}
                    onChange={(e) => update('matchTenantId', e.target.value)}
                    placeholder="tenant-…"
                    mono
                  />
                </Field>
                ...
                <Field>
                  ...
                  <select
                    value={form.matchModality}
                    onChange={(e) => update('matchModality', e.target.value as FormState['matchModality'])}
                    className="h-9 rounded-lg border border-border bg-surface-2 px-3 text-sm text-fg focus:outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/20"
                  >
