...
const EMPTY = { name: '', scopes: ['chat:read', 'chat:write'] };
export function CreateApiKeyDialog({ open, onOpenChange, tenantId, tenantName, onCreated, }) {
    ...
    return (<Dialog open={open} onOpenChange={handleClose}>
      <DialogContent size="lg">
        {createdKey ? (<div className="flex flex-col min-h-0">
            <DialogHeader>
            ...
            <DialogBody className="space-y-4">
              ...
              <Field>
                ...
                <div className="flex items-center gap-2">
                  <Input value={createdKey} readOnly mono className="font-mono"/>
                  <Button type="button" variant="secondary" size="md" onClick={handleCopy} leftIcon={copied ? (<Check className="size-3.5 text-success"/>) : (<Copy className="size-3.5"/>)}>
                    {copied ? 'Copied' : 'Copy'}
                  </Button>
                </div>
              </Field>
  ...
  -H "Authorization: Bearer ${createdKey.slice(0, 12)}…"`}
          ...
          </div>) : (<form onSubmit={handleSubmit} className="flex flex-col min-h-0 max-h-[90vh]">
            <DialogHeader>
              <div className="flex items-center gap-2 mb-1">
            ...
            <DialogBody className="space-y-4">
              <Field>
                ...
                <Input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Production key" invalid={!!errors.name} autoFocus/>
              ...
              <Field>
                ...
                return (<button type="button" key={s.id} onClick={() => toggleScope(s.id)} className={`flex items-start gap-2 rounded-lg border p-2.5 text-left transition-colors ${active
                        ? 'border-primary/40 bg-primary/5'
                        : 'border-border bg-surface-2/40 hover:border-border-strong'}`}>
                        <div className={`mt-0.5 size-3.5 rounded border flex items-center justify-center shrink-0 ${active ? 'bg-primary border-primary' : 'border-border-strong'}`}>
                ...
                <div className="flex items-center gap-1.5 flex-wrap pt-1">
                  ...
                  {form.scopes.length === 0 ? (<span className="text-[10px] text-fg-subtle">none</span>) : (form.scopes.map((s) => (<Badge key={s} tone="primary" size="sm">
                        {s}
                        <button type="button" onClick={() => toggleScope(s)} className="ml-1 hover:text-fg" aria-label={`Remove ${s}`}>
                          <X className="size-2"/>
                        </button>
                      </Badge>)))}
                </div>
...
//# sourceMappingURL=CreateApiKeyDialog.js.map