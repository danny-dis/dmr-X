...
export function AddProviderDialog({ open, onOpenChange, template, onCreated, }) {
    ...
    React.useEffect(() => {
        if (open) {
            setErrors({});
            if (template) {
                const preset = ADAPTER_PRESETS.find((p) => p.id === template.id || p.label.toLowerCase() === template.name.toLowerCase());
                setForm({
                    name: template.name,
    ...
    const onAdapterChange = (id) => {
        ...
        if (errors.adapterType)
            setErrors((prev) => ({ ...prev, adapterType: undefined }));
    };
    const validate = () => {
        const next = {};
        if (!form.name.trim())
            next.name = 'Name is required';
        if (!form.adapterType.trim())
            next.adapterType = 'Adapter is required';
        ...
        <form onSubmit={handleSubmit} className="flex flex-col min-h-0 max-h-[90vh]">
          ...
          <DialogBody className="space-y-4">
            <Field>
              ...
              <Input value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="My OpenAI account" invalid={!!errors.name} autoFocus/>
              {errors.name && <FieldError>{errors.name}</FieldError>}
            </Field>

            <Field>
              <FieldLabel required>Adapter</FieldLabel>
              <select value={form.adapterType} onChange={(e) => onAdapterChange(e.target.value)} className="h-9 rounded-lg border border-border bg-surface-2 px-3 text-sm text-fg focus:outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/20">
                {ADAPTER_PRESETS.map((p) => (<option key={p.id} value={p.id}>
                    {p.label}
                  </option>))}
              </select>
            ...
            <Field>
              ...
              <Input value={form.baseUrl} onChange={(e) => update('baseUrl', e.target.value)} placeholder="https://api.openai.com/v1" mono/>
              <FieldDescription>
            ...
            <Field>
              ...
              <Input type="password" value={form.apiKey} onChange={(e) => update('apiKey', e.target.value)} placeholder="sk-…" autoComplete="off"/>
              <FieldDescription>
            ...
            <Field>
              ...
              <Input type="password" value={form.oauthAccessToken} onChange={(e) => update('oauthAccessToken', e.target.value)} placeholder="Bearer token" autoComplete="off"/>
              <FieldDescription>
...
//# sourceMappingURL=AddProviderDialog.js.map