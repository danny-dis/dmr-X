...
import { Database, Brain, Layers, Gauge, Eye, Wrench, Sparkles, Hash, Calendar, Trash2, CheckCircle2, Circle, Cpu, } from 'lucide-react';
...
export function ModelDetailDrawer({ model, open, onOpenChange, onChanged, }) {
      ...
      <DrawerContent side="right" size="lg">
        <DrawerHeader>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge tone={isActive ? 'success' : 'muted'} size="sm" icon={isActive ? <CheckCircle2 className="size-2.5"/> : <Circle className="size-2.5"/>}>
        ...
        <DrawerBody className="space-y-5">
          <section>
            ...
            <div className="rounded-lg border border-border bg-surface-2/40 px-3 divide-y divide-border/0">
              ...
              <MetaRow label="Layer" value={model.intelligenceLayer ?? <Empty />}/>
              <MetaRow label="Tier" value={model.tier ? <Badge tone="muted" size="sm">{model.tier}</Badge> : <Empty />}/>
              {model.qualityScore != null && (<MetaRow label="Quality" value={<div className="flex items-center gap-1.5">
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              ...
              <Switch checked={isActive} onCheckedChange={toggleActive} disabled={saving === 'isActive'} aria-label="Active"/>
            </div>
            ...
            return (<label key={cap.key} className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-surface-2/60 transition-colors">
                    ...
                    <Switch checked={value} onCheckedChange={(v) => updateCapability(cap.key, v)} disabled={loading} aria-label={cap.label}/>
                  </label>);
        })}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="rounded-lg border border-border bg-surface-2/40 px-3">
              <MetaRow label="Context window" value={model.contextWindow != null
            ? formatNumber(model.contextWindow, true) + ' tokens'
            : <Empty />}/>
              <MetaRow label="Max output" value={model.maxOutputTokens != null
          ...
          <section>
            ...
            <div className="rounded-lg border border-border bg-surface-2/40 px-3">
              <MetaRow label="Input / 1k" value={model.inputCostPer1k != null ? (formatCurrency(model.inputCostPer1k)) : (<Empty />)}/>
              <MetaRow label="Output / 1k" value={model.outputCostPer1k != null ? (formatCurrency(model.outputCostPer1k)) : (<Empty />)}/>
              {model.costPerImage != null && (<MetaRow label="Per image" value={formatCurrency(model.costPerImage)}/>)}
            </div>
          </section>

          {model.createdAt && (<section>
              <h3 className="text-[10px] font-semibold text-fg-subtle uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <Calendar className="size-3"/>
                Metadata
              </h3>
              <div className="rounded-lg border border-border bg-surface-2/40 px-3">
                <MetaRow label="Created" value={formatDateTime(model.createdAt)}/>
              </div>
            </section>)}
        </DrawerBody>
...
//# sourceMappingURL=ModelDetailDrawer.js.map