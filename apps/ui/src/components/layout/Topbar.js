...
export function Topbar() {
    ...
    const { data: health } = useApiData(() => Admin.health(), [], { refetchInterval: 10_000 });
    const statusColor = health?.status === 'ok' || health?.status === 'operational'
        ? '#34D399'
        : health?.status === 'degraded'
            ? '#FBBF24'
            : '#F87171';
    return (<header className="flex h-14 items-center gap-2 border-b border-border bg-surface-1/40 px-3 backdrop-blur sm:gap-3 sm:px-5">
      <div className="flex min-w-0 flex-1 items-center gap-2 md:flex-none">
        {page && (<>
            ...
            {page.description && (<span className="text-[11px] text-fg-muted hidden sm:inline truncate">· {page.description}</span>)}
          </>)}
      </div>

      <div className="mx-auto hidden max-w-md flex-1 md:block">
        <button onClick={() => setCommandPaletteOpen(true)} className={cn('w-full flex items-center gap-2 h-8 px-3 rounded-lg border border-border bg-surface-2/60 text-fg-muted text-xs', 'hover:border-border-strong hover:text-fg transition-colors')}>
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
        <button onClick={() => setLiveMode(!liveMode)} className={cn('flex h-7 items-center gap-1.5 rounded-md border px-2 text-[11px] font-medium transition-colors', liveMode
        ...
        <Button size="icon-sm" variant="ghost" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} aria-label="Toggle theme">
