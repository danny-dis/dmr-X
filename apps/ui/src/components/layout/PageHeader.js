...
export function PageHeader({ title, description, icon, actions, breadcrumbs, className, }) {
    const location = useLocation();
    const group = findGroup(location.pathname);
    const crumbs = breadcrumbs ??
        (group ? [{ label: group.label }, { label: title }] : [{ label: title }]);
    return (<div className={cn('flex flex-col gap-2 pb-4 border-b border-border', className)}>
      <nav className="flex items-center gap-1 text-[11px] text-fg-muted">
        {crumbs.map((c, i) => (<React.Fragment key={i}>
            ...
            {c.path ? (<Link to={c.path} className="hover:text-fg transition-colors">
                {c.label}
              </Link>) : (<span className={i === crumbs.length - 1 ? 'text-fg' : ''}>{c.label}</span>)}
          </React.Fragment>))}
      </nav>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 min-w-0">
          {icon && (<div className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-border bg-surface-2 text-primary">
              {icon}
            </div>)}
