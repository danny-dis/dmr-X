export function formatCurrency(n, currency = 'USD', maximumFractionDigits = 4) {
    if (Math.abs(n) >= 1) {
...
export function formatCompactCurrency(n, currency = 'USD') {
    if (n >= 1000) {
...
export function formatTokens(n, compact = false) {
    if (!compact) {
...
export function formatNumber(n, compact = false, decimals = 1) {
...
export function formatRate(n, unit, perSec = true) {
    return `${formatNumber(n, true)} ${unit}${perSec ? '/s' : ''}`;
}
export function formatLatency(p50, p95, p99) {
...
export function maskKey(key, visible = 4) {
    if (!key)
        return '';
