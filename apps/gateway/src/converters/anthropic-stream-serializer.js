 ...
 */
export async function* createAnthropicSSEStream(chunks, options) {
    let started = false;
    let blockStarted = false;
    let outputTokens = 0;
    const formatEvent = (eventType, data) => `event: ${eventType}\ndata: ${JSON.stringify(data)}\n\n`;
    for await (const chunk of chunks) {
        if (chunk.type === 'token') {
            ...
            const stopReason = ANTHROPIC_STOP_REASON_MAP[finishReason] ?? 'end_turn';
...
//# sourceMappingURL=anthropic-stream-serializer.js.map