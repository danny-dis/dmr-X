...
import { registryService, HealthChecker, PROVIDER_CATALOG, autoRegisterProviders, discoverMissingModels } from '@dmr-x/registry';
...
export async function createServer() {
    ...
    const existing = tenantDb.prepare("SELECT id FROM tenants WHERE name = ?").get(defaultTenantName);
    if (!existing) {
    ...
    const registeredProviders = db.prepare('SELECT * FROM providers').all();
            ...
            if (row.oauth_token_expires_at && new Date(row.oauth_token_expires_at) < new Date(Date.now() + 60000)) {
                if (row.oauth_refresh_token && template?.oauthConfig) {
                    try {
                        ...
                        db.prepare(`UPDATE providers SET oauth_access_token = ?, oauth_refresh_token = ?, oauth_token_expires_at = ?, updated_at = datetime('now') WHERE id = ?`).run(encAccess, encRefresh, newTokens.expiresAt?.toISOString() || null, row.id);
    ...
    const allProviders = db.prepare(`SELECT id, name, config, is_healthy, auth_method, oauth_access_token FROM providers`).all();
    for (const p of allProviders) {
        ...
        if (needsNoKey && !p.is_healthy) {
            db.prepare(`UPDATE providers SET is_healthy = 1, consecutive_failures = 0, updated_at = datetime('now') WHERE id = ?`).run(p.id);
        ...
        if (cfg.hasKey || hasOAuthToken || needsNoKey) {
            const updated = db.prepare(`UPDATE model_profiles SET is_active = 1, updated_at = datetime('now')
         WHERE provider_id = ? AND is_active = 0`).run(p.id);
    ...
    server.decorate('getAdapter', (providerId) => {
        ...
        if (!adapter) {
            const row = db.prepare('SELECT name FROM providers WHERE id = ?').get(providerId);
            if (row)
                adapter = adapterRegistry.get(row.name);
        }
        return adapter;
    });
    router.setAdapterExecutor({
        execute: async (providerId, _modelId, request) => {
            const adapter = server.getAdapter(providerId);
            if (!adapter)
    ...
    const oauthRefreshTimer = setInterval(async () => {
        try {
            const rows = db.prepare(`SELECT id, name, oauth_refresh_token, oauth_token_expires_at
         FROM providers
         WHERE auth_method = 'oauth'
         AND oauth_token_expires_at IS NOT NULL
         AND oauth_refresh_token IS NOT NULL`).all();
            for (const row of rows) {
                ...
                if (expiresAt.getTime() < Date.now() + bufferMs) {
                    ...
                    try {
                        ...
                        const adapter = adapterRegistry.get(row.name);
                        if (adapter) {
                            const providerRow = db.prepare('SELECT base_url FROM providers WHERE id = ?').get(row.id);
    ...
    const uiDir = candidateDirs.find(d => { try {
    ...
    server.addHook('onSend', async (_request, reply, payload) => {
        ...
        reply.header('Content-Security-Policy', "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: *; connect-src 'self'");
