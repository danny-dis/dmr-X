...
import { generateRequestId, stepCountIs, hasToolCall, isStopConditionMet, createInitialState, updateState, logger, } from '@dmr-x/utils';
...
if (conversationCleanupTimer.unref)
...
// ---------------------------------------------------------------------------
function buildStopConditions(conditions, getResponseText) {
    return conditions.map((c) => {
        switch (c.type) {
            case 'step_count':
                return stepCountIs(c.value);
            case 'tool_call':
                return hasToolCall(c.value);
            case 'text_match': {
                const text = c.value;
...
// Route registration
// ---------------------------------------------------------------------------
export async function agenticRoutes(server) {
    /**
     * POST /agentic/chat
     *
    ...
    server.post('/agentic/chat', async (request, reply) => {
        ...
        let lockResolver;
        const lockPromise = new Promise((resolve) => { lockResolver = resolve; });
        conversationLocks.set(convId, lockPromise);
        const releaseLock = () => { lockResolver?.(); if (conversationLocks.get(convId) === lockPromise)
        ...
        try {
            ...
            for (let turn = 0; turn < maxSteps; turn++) {
                ...
                if (lastStep)
                    lastStep.tool_results = stepResults;
                // Add tool results to messages
                for (const tr of stepResults) {
                    messages.push({
                        role: 'tool',
