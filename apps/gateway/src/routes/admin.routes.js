...
import { logger, encrypt, decrypt, encryptConfigApiKey } from '@dmr-x/utils';
...
}).refine((obj) => !Object.keys(obj).some((k) => BLOCKED_SETTINGS_KEYS.has(k)), { message: 'Blocked key detected' });
// SSRF validation: block private/internal IP ranges and non-http(s) protocols
function validateBaseUrlForSSRF(urlStr) {
    let parsedUrl;
    try {
...
export async function adminRoutes(server) {
    async function createApiKeyForTenant(tenantId, name) {
        const db = getDb();
        ...
        db.prepare('INSERT INTO api_keys (id, tenant_id, key_hash, name) VALUES (?, ?, ?, ?)').run(id, tenantId, keyHash, name);
        const row = db.prepare('SELECT id, tenant_id, name, created_at FROM api_keys WHERE id = ?').get(id);
        return {
            ...row,
            key: apiKey,
        };
    ...
    server.post('/admin/providers/activate', async (request, reply) => {
        ...
        const { template_id, api_key, oauth_access_token, oauth_refresh_token, oauth_token_expires_at, auth_method, name: custom_name, } = parsed.data;
        ...
        let provider = db.prepare('SELECT * FROM providers WHERE name = ?').get(providerName);
        ...
        if (provider) {
         ...
         WHERE id = ?`).run(template.baseUrl, encKey, hasApiKey ? 1 : 0, encryptedOAuthAccessToken, encryptedOAuthRefreshToken, oauth_token_expires_at ?? null, requestedAuthMethod, provider.id);
            // Activate models for this provider
            if (shouldActivateModels) {
                const activated = db.prepare(`UPDATE model_profiles SET is_active = 1, updated_at = datetime('now')
           WHERE provider_id = ? AND is_active = 0`).run(provider.id);
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, providerName, template.id, template.baseUrl, template.envKey || '', JSON.stringify(configObj), encryptedOAuthAccessToken, encryptedOAuthRefreshToken, oauth_token_expires_at ?? null, requestedAuthMethod);
            ...
            for (const model of template.models) {
                db.prepare(`INSERT INTO model_profiles (
          ...
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(crypto.randomUUID(), id, model.id, model.id, model.modalities[0] || 'llm', 'executor', model.capabilities.includes('streaming') ? 1 : 0, model.capabilities.includes('vision') ? 1 : 0, model.capabilities.includes('tool_use') ? 1 : 0, model.contextWindow, model.maxOutputTokens, (model.inputCostPer1M || 0) / 1000, (model.outputCostPer1M || 0) / 1000, model.costPerImage || 0, 0.5, shouldActivateModels ? 1 : 0);
            }
        }
        provider = db.prepare('SELECT * FROM providers WHERE name = ?').get(providerName);
        // Initialize/Update adapter in registry
        const adapterRegistry = server.adapterRegistry;
        ...
        const refreshCandidates = server.refreshCandidates;
    ...
    server.put('/admin/providers/:id/api-key', async (request, reply) => {
        ...
        db.prepare(`UPDATE providers SET config = ?, is_healthy = 1, consecutive_failures = 0, updated_at = datetime('now') WHERE id = ?`).run(JSON.stringify(config), id);
        // Activate models for this provider now that it has a key
        const activated = db.prepare(`UPDATE model_profiles SET is_active = 1, updated_at = datetime('now')
       WHERE provider_id = ? AND is_active = 0`).run(id);
        if (activated.changes > 0) {
        ...
        if (refreshCandidates)
            await refreshCandidates();
        const updatedRow = db.prepare('SELECT * FROM providers WHERE id = ?').get(id);
    ...
    server.post('/admin/providers/:id/oauth/authorize', async (request, reply) => {
        ...
        if (oauthConfig.flow === 'client_credentials') {
            ...
            consecutive_failures = 0,
            updated_at = datetime('now')
          WHERE id = ?`).run(encAccess, encRefresh, tokens.expiresAt?.toISOString() || null, id);
                // Activate models
                db.prepare(`UPDATE model_profiles SET is_active = 1, updated_at = datetime('now')
           WHERE provider_id = ? AND is_active = 0`).run(id);
    ...
    server.post('/admin/providers', async (request, reply) => {
        ...
        const db = getDb();
        const id = crypto.randomUUID();
        db.prepare(`INSERT INTO providers (id, name, adapter_type, base_url, api_key_ref, config)
       VALUES (?, ?, ?, ?, ?, ?)`).run(id, body.name, body.adapter_type, body.base_url ?? null, body.api_key_ref ?? null, JSON.stringify(configToStore));
        reply.status(201);
        const created = db.prepare('SELECT * FROM providers WHERE id = ?').get(id);
    ...
    server.get('/admin/models', async () => {
        const db = getDb();
        const rows = db.prepare(`SELECT mp.*, p.name as provider_name
    ...
    server.post('/admin/models', async (request, reply) => {
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, body.provider_id, body.model_id, body.display_name, body.modality, body.intelligence_layer, body.context_window, body.max_output_tokens, body.supports_streaming ? 1 : 0, body.supports_vision ? 1 : 0, body.supports_tool_use ? 1 : 0, body.input_cost_per_1k, body.output_cost_per_1k, body.cost_per_image);
        reply.status(201);
    ...
    server.post('/admin/api-keys', async (request, reply) => {
        ...
        const keyHash = hashApiKey(apiKey);
        const id = crypto.randomUUID();
        db.prepare('INSERT INTO api_keys (id, tenant_id, key_hash, name) VALUES (?, ?, ?, ?)').run(id, tenant_id, keyHash, name);
        const row = db.prepare('SELECT id, tenant_id, name, created_at FROM api_keys WHERE id = ?').get(id);
        reply.status(201);
        return {
            ...row,
    ...
    server.post('/admin/policies', async (request, reply) => {
        ...
        db.prepare(`INSERT INTO policies (id, tenant_id, name, rules, is_active, created_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'))`).run(id, body.tenant_id, body.name, rulesBlob, body.enabled ? 1 : 0);
        reply.status(201);
        return {
            id,
    ...
    server.telemetryBuffer = telemetryBuffer;
    server.trimTelemetryBuffer = trimTelemetryBuffer;
    // Audit events
    ...
    server.post('/admin/memory', async (request, reply) => {
        const { tenantId, content, namespace, source, retentionDays, metadata } = request.body;
        if (!content) {
            reply.status(400);
    ...
    server.post('/admin/memory/search', async (request) => {
        const { tenantId, query, namespace, limit, minScore } = request.body;
    ...
    server.post('/admin/sandbox/jobs', async (request, reply) => {
        const { tenantId, language, code, timeoutMs, maxRetries } = request.body;
        if (!code) {
            reply.status(400);
    ...
    server.post('/admin/federation', async (request, reply) => {
        const { name, url, region, apiKey, privacyLevel } = request.body;
        if (!name || !url) {
            reply.status(400);
    ...
    server.put('/admin/providers/:id', async (request, reply) => {
        ...
        const { name, adapter_type, base_url, api_key_ref, config, auth_method, oauth_access_token, oauth_refresh_token, oauth_token_expires_at, } = parsed.data;
        // Issue #9: SSRF validation on base_url
        if (base_url) {
            validateBaseUrlForSSRF(base_url);
        }
        ...
        updated_at = datetime('now')
      WHERE id = ?`).run(name ?? null, adapter_type ?? null, base_url ?? null, api_key_ref ?? null, configToStore ? JSON.stringify(configToStore) : null, encryptedOAuthAccessToken, encryptedOAuthRefreshToken, oauth_token_expires_at ?? null, auth_method ?? (oauth_access_token ? 'oauth' : null), oauth_access_token ?? null, oauth_access_token ?? null, id);
        ...
        if (oauth_access_token) {
            const activated = db.prepare(`UPDATE model_profiles SET is_active = 1, updated_at = datetime('now')
         WHERE provider_id = ? AND is_active = 0`).run(updated.id);
            if (activated.changes > 0) {
    ...
    server.put('/admin/models/:id', async (request, reply) => {
        ...
        const db = getDb();
        db.prepare(`UPDATE model_profiles SET
        ...
        updated_at = datetime('now')
      WHERE id = ?`).run(display_name, modality, context_window, max_output_tokens, is_active != null ? (is_active ? 1 : 0) : null, id);
    ...
    server.put('/admin/policies/:id', async (request) => {
        ...
        const db = getDb();
        const existing = db.prepare('SELECT rules FROM policies WHERE id = ?').get(id);
        if (!existing) {
        ...
        is_active = COALESCE(?, is_active),
        updated_at = datetime('now')
      WHERE id = ?`).run(name ?? null, JSON.stringify(updatedRules), enabled != null ? (enabled ? 1 : 0) : null, id);
