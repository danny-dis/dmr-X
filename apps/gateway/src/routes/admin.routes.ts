...
COALESCE(
   ...
   *
   * `telemetryBuffer` is process-memory only (see admin-events.ts) and —
   * separately — `recordTelemetryEvent` is only ever called from error/warning
   * code paths (auth failures, routing failures, rate limits), never for an
   * ordinary completed request. So on a fresh gateway process the Requests
   * page's "Live stream of all requests" read an empty buffer and showed "No
