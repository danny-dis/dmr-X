...
export async function authMiddleware(server) {
    ...
    server.addHook('onRequest', async (request) => {
        ...
        const db = getDb();
        const row = db.prepare(`SELECT ak.id, ak.tenant_id, t.name as tenant_name
       FROM api_keys ak
       JOIN tenants t ON t.id = ak.tenant_id
       WHERE ak.key_hash = ? AND ak.is_active = 1`).get(keyHash);
        if (!row) {
        ...
        const now = Date.now();
        const lastUsed = request.tenant?.lastUsedAt;
        if (!lastUsed || now - lastUsed > 5 * 60 * 1000) {
            db.prepare("UPDATE api_keys SET last_used_at = datetime('now') WHERE id = ?").run(row.id);
            request.tenant.lastUsedAt = now;
        }
    });
}
