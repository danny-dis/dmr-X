...
export function isConnectionError(err: unknown): boolean {
  ...
  // Bun
  const isBunErr = 'name' in err && (err as any).name === 'ConnectionError';

  // Generic (Node.js http, Axios)
  const isGenericErr =
    'code' in err &&
    typeof (err as any).code === 'string' &&
    (err as any).code.toLowerCase() === 'econnreset';
...
export function isTimeoutError(err: unknown): boolean {
  ...
  const isNative = 'name' in err && (err as any).name === 'TimeoutError';
  const isLegacyNative = 'code' in err && (err as any).code === 23;
