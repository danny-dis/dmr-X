...
export function decrypt(encryptedHex) {
    const key = getEncryptionKey();
    if (!key)
        return encryptedHex; // graceful fallback
 ...
 */
export function encryptConfigApiKey(config) {
 ...
 */
export function decryptConfigApiKey(config) {
    if (typeof config.apiKey === 'string' && config.apiKey.length > 0) {
        try {
 ...
 */
export function encryptOAuthTokens(config) {
 ...
 */
export function decryptOAuthTokens(config) {
    if (typeof config.oauthAccessToken === 'string' && config.oauthAccessToken.length > 0) {
        try {
