...
const log = {
    info: (...args) => console.log('[dmr-x]', ...args),
    error: (...args) => console.error('[dmr-x]', ...args),
    warn: (...args) => console.warn('[dmr-x]', ...args),
};
let db = null;
let dbPath = '';
...
// ---------------------------------------------------------------------------
async function migratePlaintextApiKeys(dbWrapper) {
...
export async function initDb() {
  ...
  `);
    ...
    for (const mig of [...migrations.values()].sort((a, b) => a.version - b.version)) {
        ...
        try {
            db.exec(mig.sql);
            const insertStmt = db.prepare('INSERT OR IGNORE INTO schema_version (version, filename) VALUES (?, ?)');
            ...
            if (msg.includes('duplicate column name')) {
                ...
                const retryStmt = db.prepare('INSERT OR IGNORE INTO schema_version (version, filename) VALUES (?, ?)');
