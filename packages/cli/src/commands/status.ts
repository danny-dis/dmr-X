...
async function checkLocalEndpoint(url: string, timeoutMs: number = 3000): Promise<{ ok: boolean; latencyMs: number; error?: string }> {
...
async function checkProviderHealth(provider: RegisteredProvider): Promise<HealthCheckResult> {
...
export function createStatusCommand(): Command {
  return new Command('status')
    ...
    .action(async (opts) => {
      ...
      console.log(
        chalk.white('  Project:     '),
        initialized ? chalk.green('Initialized') : chalk.red('Not initialized')
      );
      ...
      console.log(
        chalk.white('  docker-compose: '),
        hasDockerCompose ? chalk.green('found') : chalk.gray('not found')
      );

      const envPath = getEnvPath();
      const hasEnv = existsSync(envPath);
      console.log(
        chalk.white('  .env file:      '),
        hasEnv ? chalk.green('found') : chalk.gray('not found')
      );
      ...
      if (opts.health) {
        ...
        for (const svc of localServices) {
          ...
          spinner.stop();
          console.log(
            chalk.white(`  ${svc.name.padEnd(14)}`),
            result.ok
