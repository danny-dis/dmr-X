...
function formatModels(models, max = 3) {
    if (models.length === 0)
...
function isProviderRegistered(providerId, registered) {
    return registered.some((p) => p.id === providerId && p.enabled);
}
export function createListProvidersCommand() {
    return new Command('list')
        ...
        for (const cat of categories) {
            ...
            for (const p of catProviders) {
                ...
                const badges = [];
                if (isRegistered)
                    badges.push(chalk.green('registered'));
                if (isLocal)
                    badges.push(chalk.blue('local'));
                if (hasFree)
                    badges.push(chalk.yellow('free'));
