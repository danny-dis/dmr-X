...
export const TOOL_DESCRIPTIONS = {
    dmrx_chat: 'Send a chat completion request through DMR-X. Automatically routes to the best available LLM ' +
        'based on quality, cost, and latency targets. Supports vision, tool use, and all OpenAI-compatible parameters.',
    dmrx_generate_image: 'Generate images through DMR-X. Automatically routes to the best available diffusion model ' +
        '(Stable Diffusion, DALL-E, Replicate, etc.). Supports text-to-image and style presets.',
    dmrx_embed: 'Get text embeddings through DMR-X. Routes to the best embedding model for the given input. ' +
        'Supports single strings and batches.',
    dmrx_transcribe: 'Transcribe audio to text through DMR-X. Routes to the best STT model (OpenAI Whisper, Deepgram, etc.). ' +
        'Accepts base64-encoded audio or URLs.',
    dmrx_speak: 'Convert text to speech through DMR-X. Routes to the best TTS model (ElevenLabs, OpenAI, etc.). ' +
        'Supports multiple voices and speed control.',
    dmrx_rerank: 'Rerank documents by relevance to a query through DMR-X. Routes to the best reranking model ' +
        '(Cohere, Jina, etc.). Useful for RAG pipelines.',
    dmrx_models: 'List available models in DMR-X, optionally filtered by modality or provider. ' +
        'Shows model capabilities, health status, and cost information.',
    dmrx_status: 'Get DMR-X system status including router health, provider availability, and configuration. ' +
        'Useful for diagnostics and monitoring.',
    dmrx_batch: 'Execute multiple MCP tool calls atomically. Returns aggregated results with individual outcomes. ' +
        'Supports partial failure modes (continue-on-fail vs fail-fast).',
    dmrx_context_save: 'Save conversation context with a persistent ID. Enables stateful agent interactions across sessions. ' +
        'Accepts TTL for automatic expiry.',
    dmrx_context_load: 'Load a previously saved conversation context by ID. Returns messages and metadata.',
    dmrx_context_list: 'List saved conversation contexts, optionally filtered by user. Supports pagination.',
    dmrx_context_summarize: 'Generate a contextual summary of a saved conversation. Reduces token cost for long conversations.',
    dmrx_context_compress: 'Compress a saved conversation context while preserving meaning. Reduces storage and token usage.',
    dmrx_chat_stream: 'Streaming chat completion through DMR-X. Returns token-by-token output via streaming response.',
    dmrx_generate_image_stream: 'Streaming image generation through DMR-X. Returns progressive generation updates.',
    dmrx_workflow: 'Define and execute multi-step workflows. Supports conditional branching, looping, parallel execution, ' +
