...
import {
  ...
  dmrxStatusParams,
  dmrxBatchParams,
  dmrxContextSaveParams,
  dmrxContextLoadParams,
  dmrxContextListParams,
  dmrxContextSummarizeParams,
  dmrxContextCompressParams,
  dmrxChatStreamParams,
  dmrxGenerateImageStreamParams,
  dmrxWorkflowParams,
} from './tools.js';
...
export function createMCPPlugin(): Plugin {
  let deps: PluginDependencies | null = null;
  let serverReturnValue: { server: McpServer; state: any } | null = null;
  ...
  return {
    ...
    get manifest() {
      return {
        ...
        transport: { type: 'stdio' as const }, // default; configurable via start()
        permissions: {
          accessModalities: [
    ...
    async init(injectedDeps: PluginDependencies) {
      deps = injectedDeps;
    ...
    buildAdapterRegistry(): AdapterRegistry {
    ...
    async initAdapters(adapterRegistry: AdapterRegistry) {
    ...
    createMCPServerInstance(config: {
      ...
      const state = {
        ...
        requestCount: 0,
        lastError: null,
        sdkTools: [] as Array<{ name: string; description: string; params: unknown }>,
      };
    ...
    registerMcpTools(server: McpServer, state: any, initAdapters: () => Promise<void>) {
        ...
        async (params: any) => {
          ...
          try {
            ...
            const classifyOptions: ClassifyOptions = {
              ...
              qualityTarget: (params.quality_target as any) || 'balanced',
            };
    ...
    toUnifiedRequest(modality: Modality, params: Record<string, unknown>): UnifiedRequest {
    ...
    formatChatResponse(response: UnifiedResponse): string {
    ...
    formatRoutingInfo(response: UnifiedResponse): string {
      return `\n\n---\nRouted via: ${response.providerId} / ${response.modelId} (${response.latencyMs}ms)`;
    },

    // Transport starters (adapted from index.ts)
    async startStdio(server: McpServer): Promise<void> {
      const { StdioServerTransport } = await import('@modelcontextprotocol/sdk/server/stdio.js');
      const transport = new StdioServerTransport();
      await server.connect(transport);
    },

    async startSSE(server: McpServer, port: number, host: string, apiKey: string, deps: PluginDependencies): Promise<void> {
      // Dynamically import to avoid pulling in HTTP deps for stdio-only usage
      const { SSEServerTransport } = await import('@modelcontextprotocol/sdk/server/sse.js');
      const http = await import('node:http');

      const sessions = new Map<string, { server: McpServer; transport: any }>();

      const checkAuth = (req: { headers: Record<string, string | string[] | undefined> }, res: { writeHead: (status: number, headers?: Record<string, string>) => void; end: (body: string) => void }): boolean => {
    ...
    async startStreamableHTTP(server: McpServer, port: number, host: string, apiKey: string, deps: PluginDependencies): Promise<void> {
    ...
    registerToolHandlersIfAvailable(deps: PluginDependencies) {
