    ...
    async storeResults(results) {
        ...
        for (const result of results) {
            try {
                ...
                const modelRow = db.prepare(`SELECT mp.id FROM model_profiles mp
           JOIN providers p ON p.id = mp.provider_id
           WHERE p.name = ? AND mp.model_id = ?`).get(result.providerId, result.modelId);
                if (modelRow) {
                    db.prepare(`INSERT INTO benchmark_results (model_id, benchmark_type, score, details)
             VALUES (?, ?, ?, ?)`).run(modelRow.id, result.benchmarkType, result.score, JSON.stringify(result.details));
            ...
            WHERE id = ?`).run(modelRow.id, result.latencyMs, modelRow.id);
                }
            }
            catch (error) {
