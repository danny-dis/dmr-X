...
import { logger, DefaultHttpHooks, createHttpError, HttpError, withRetry, TemporaryError, PermanentError, isConnectionError, isTimeoutError, isAbortError, ConnectionError, ClientTimeoutError, match, } from '@dmr-x/utils';
/**
...
export class BaseAdapter {
     ...
     */
    handleAdapterError(error, context) {
        if (error instanceof ProviderError)
            throw error;
        const prefix = context ? `${this.providerId} ${context}` : this.providerId;
        if (error instanceof HttpError) {
            const providerError = new ProviderError(`${prefix}: ${error.message}`, this.providerId, error.statusCode);
            providerError.cause = error;
            throw providerError;
        }
        if (error instanceof ConnectionError || isConnectionError(error)) {
            throw new ProviderError(`${prefix}: Connection failed - ${error instanceof Error ? error.message : String(error)}`, this.providerId, 502);
        }
     ...
     */
    async matchResponse(response, request, ...matchers) {
        return match(...matchers)(response, request);
    }
    /**
    ...
    async rawFetch(url, options = {}) {
        ...
        try {
            ...
            if (response.ok) {
                response = await this.hooks.executeAfterSuccess({ ...hookCtx, status: response.status }, response);
            }
            else {
                ...
                const result = await this.hooks.executeAfterError({ ...hookCtx, status: response.status, error: httpError }, response, httpError);
                if (result.response) {
            ...
            if (error instanceof DOMException && error.name === 'AbortError') {
                throw new ClientTimeoutError(`Request to ${url} timed out after ${timeoutMs}ms`, { cause: error });
            }
            if (error instanceof TypeError) {
                throw new ConnectionError(`Failed to connect to ${url}: ${error.message}`, { cause: error });
            }
            throw error;
        }
        finally {
    ...
    async fetchWithTimeout(url, options = {}) {
        ...
        let lastRetryableHttpError;
        try {
            return await withRetry(async () => {
                attempt++;
                if (attempt > 1) {
                    logger.info({ providerId: this.providerId, attempt, url }, 'Retrying HTTP request after transient failure');
                }
                try {
                    ...
                    if (error instanceof HttpError &&
                        ...
                        throw new TemporaryError(`Retryable HTTP ${error.statusCode}: ${error.message}`, error.rawResponse);
                    }
                    ...
                    if (error instanceof HttpError) {
                        throw new PermanentError(`Permanent HTTP ${error.statusCode}: ${error.message}`, { cause: error });
                    }
