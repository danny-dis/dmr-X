...
export class MCPClient {
    ...
    async connect(config) {
        ...
        for (const serverConfig of config.servers) {
            try {
                ...
                logger.error({ serverId: serverConfig.id, error }, 'Failed to connect to MCP server');
    ...
    async connectServer(serverConfig) {
        ...
        await adapter.initialize({ baseUrl: '' });
        this.adapters.set(serverConfig.id, adapter);
        logger.info({ serverId: serverConfig.id, toolCount: server.tools.length }, 'MCP server adapter ready');
        return server;
    }
    /**
