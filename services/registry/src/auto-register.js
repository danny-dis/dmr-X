...
import { PROVIDER_CATALOG, getProviderTemplate } from './provider-catalog.js';
/**
...
export async function autoRegisterProviders() {
    ...
    for (const template of PROVIDER_CATALOG) {
        ...
        const existing = await pool.query('SELECT id FROM providers WHERE name = $1', [template.id]);
        ...
        try {
            // Create provider
            const providerResult = await pool.query(`INSERT INTO providers (name, adapter_type, base_url, api_key_ref, config)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`, [
                template.id,
                template.id,
            ...
            for (const model of template.models) {
                await pool.query(`INSERT INTO model_profiles (
            ...
            logger.info({ provider: template.id, models: template.models.length }, 'Auto-registered provider');
        }
        catch (error) {
 ...
 */
export async function registerProvider(providerId, overrides) {
    ...
    const existing = await pool.query('SELECT id FROM providers WHERE name = $1', [merged.id]);
    if (existing.rows.length > 0) {
 ...
 */
export async function discoverHuggingFaceModels(task, limit = 20) {
    try {
        const response = await fetch(`https://huggingface.co/api/models?pipeline_tag=${task}&sort=downloads&direction=-1&limit=${limit}`);
        if (!response.ok) {
