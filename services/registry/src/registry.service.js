...
export class RegistryService {
    ...
    updateHealth(providerId, healthy, latencyMs) {
        const db = getDb();
        if (healthy) {
        ...
        // Record health check
        db.prepare('INSERT INTO health_checks (id, provider_id, is_healthy, latency_ms) VALUES (?, ?, ?, ?)').run(crypto.randomUUID(), providerId, healthy ? 1 : 0, latencyMs ?? null);
    }
