...
export class RegistryService {
    ...
    extractCapabilities(row) {
        const caps = [];
        if (row.supports_streaming)
            caps.push('streaming');
        if (row.supports_vision)
            caps.push('vision');
        if (row.supports_tool_use)
            caps.push('tool_use');
    ...
    async updateHealth(providerId, healthy, latencyMs) {
        const pool = getPool();
        if (healthy) {
            await pool.query(`UPDATE providers SET
          is_healthy = true,
        ...
        // Record health check
        await pool.query('INSERT INTO health_checks (provider_id, is_healthy, latency_ms) VALUES ($1, $2, $3)', [providerId, healthy, latencyMs]);
    }
