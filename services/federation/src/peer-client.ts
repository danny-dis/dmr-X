...
export class PeerClient {
  ...
  async request(url: string, req: PeerRequest): Promise<PeerResponse> {
  ...
  async health(url: string): Promise<PeerResponse> {
    return this.request(url, { method: 'GET', path: '/health', timeoutMs: 5000 });
  }

  async models(url: string): Promise<PeerResponse> {
    return this.request(url, { method: 'GET', path: '/v1/models' });
  }

  async chat(url: string, body: unknown): Promise<PeerResponse> {
    return this.request(url, { method: 'POST', path: '/v1/chat/completions', body });
  }

  async benchmarks(url: string): Promise<PeerResponse> {
