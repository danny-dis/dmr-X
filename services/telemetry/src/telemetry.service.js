...
import { requestCount, requestLatency, ttftLatency, tokenUsage, costEstimate, errorCount, setProviderHealthStatus, getAllProviderHealth, } from './metrics.js';
...
export class TelemetryService {
    ...
    async start() {
        ...
        if (this.config.enableMetrics) {
            ...
            logger.info({ port: this.config.metricsPort, path: this.config.metricsPath }, 'Prometheus metrics exporter started');
        }
        // OTLP trace exporter
    ...
    recordLatency(params) {
        ...
        logger.debug({ ...labels, latencyMs: params.latencyMs, ttftMs: params.ttftMs }, 'Latency recorded');
    }
    /**
    ...
    recordHealth(params) {
        ...
        logger.debug({ provider_id: params.providerId, healthy: params.healthy }, 'Provider health updated');
    }
