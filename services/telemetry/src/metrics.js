 ...
 */
export const requestCount = METER.createCounter('dmr_request_count', {
 ...
 */
export const requestLatency = METER.createHistogram('dmr_request_latency_ms', {
 ...
 */
export const ttftLatency = METER.createHistogram('dmr_ttft_latency_ms', {
 ...
 */
export const tokenUsage = METER.createCounter('dmr_token_usage_total', {
 ...
 */
export const costEstimate = METER.createCounter('dmr_cost_estimate_usd', {
 ...
 */
export const errorCount = METER.createCounter('dmr_error_count', {
...
export const providerHealth = METER.createObservableGauge('dmr_provider_health', {
    description: 'Provider health status (1 = healthy, 0 = unhealthy)',
    unit: 'status',
});
providerHealth.addCallback((result) => {
    for (const [providerId, status] of healthRegistry) {
        result.observe(status, { provider_id: providerId });
    }
});
// ─── Health Registry Helpers ─────────────────────────────────────────────────
export function setProviderHealthStatus(providerId, healthy) {
    healthRegistry.set(providerId, healthy ? 1 : 0);
}
export function getProviderHealthStatus(providerId) {
