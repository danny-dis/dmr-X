...
function buildFallbackChain(remaining, _primary) {
 ...
 */
function applyProviderPreferences(candidates, prefs) {
    ...
    if (prefs.quantizations && prefs.quantizations.length > 0) {
        ...
        result = result.filter((m) => {
            if (!m.quantization)
                return true; // allow unknown quantization
    ...
    if (prefs.preferredMaxLatencyMs !== undefined) {
        ...
        result = result.filter((m) => {
            if (!m.avgLatencyMs)
                return true; // allow unknown latency
    ...
    if (prefs.preferredMinThroughputTps !== undefined) {
        ...
        result = result.filter((m) => {
            ...
            if (tps === undefined)
                return true; // allow unknown throughput
 ...
 */
function applyProviderOrder(scored, order) {
