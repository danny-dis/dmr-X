...
export async function executeWithFallback(plan, request, executor, options) {
    ...
    try {
        ...
        try {
            ...
            logger.warn({ err: cbErr }, 'onSuccess callback error');
        }
        ...
        try {
            ...
            logger.warn({ err: cbErr }, 'onFailure callback error');
        }
    ...
    for (const step of plan.chain) {
        ...
        try {
            ...
            if (isQuotaError(error)) {
                ...
                logger.warn({ err: error, provider: step.provider.providerId, requestId: options?.requestId }, 'Fallback provider failed');
            }
            // On 429, add penalty and record usage
            if (rls && isRateLimitError(error)) {
