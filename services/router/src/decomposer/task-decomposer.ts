...
export class TaskDecomposer {
  ...
  private extractSubTasks(prompt: string, _detectedFlow?: string): SubTask[] {
    ...
    if (this.containsKeywords(lowerPrompt, [
      ...
      subTasks.push({
        ...
        specializations: ['frontend_logic', 'ui_component', 'architecture'],
        priority: 7,
        estimatedTokens: 2000,
        canParallel: true,
        modality: 'llm',
    ...
    if (this.containsKeywords(lowerPrompt, [
      ...
      subTasks.push({
        ...
        specializations: ['embedding', 'reasoning'],
        priority: 7,
  ...
  private detectSpecializations(prompt: string): Specialization[] {
    ...
    const keywordMap: Record<Specialization, string[]> = {
      ...
      frontend_logic: ['state', 'hook', 'context', 'redux', 'zustand', 'jotai', 'recoil', 'pinia', 'mobx', 'ios', 'android', 'swift', 'kotlin', 'react native', 'flutter', 'expo', 'capacitor', 'mobile'],
      ...
      refactoring: ['refactor', 'cleanup', 'improve', 'extract', 'split', 'consolidate', 'optimize', 'performance', 'cache', 'lazy', 'eager', 'debounce', 'throttle', 'profiling'],
      debugging: ['bug', 'error', 'fix', 'debug', 'trace', 'stacktrace', 'regression', 'race condition', 'security', 'vulnerability', 'xss', 'csrf', 'injection', 'encrypt', 'hash', 'sanitiz'],
      ...
      embedding: ['embedding', 'similarity', 'search', 'vector', 'semantic', 'rag', 'retrieval', 'model', 'train', 'inference', 'dataset', 'ml', 'ai', 'neural', 'transformer', 'llm'],
      general: [],
