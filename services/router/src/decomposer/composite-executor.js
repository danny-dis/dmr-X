...
export class CompositeExecutor {
     ...
     */
    async execute(decomposed, candidates, originalRequest, qualityTarget = 'balanced', freeTierStrategy) {
        const start = Date.now();
        const subTaskResults = new Map();
        const modelAssignments = new Map();
        // Route all sub-tasks to specialist models
        const assignments = this.specialistRouter.routeAllSubTasks(decomposed.subTasks, candidates, qualityTarget, freeTierStrategy);
        // Log assignments
     ...
     */
    aggregateResults(_decomposed, results, _originalRequest) {
