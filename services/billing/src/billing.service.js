...
export class BillingService {
    ...
    async refreshPricingCache() {
        const pool = getPool();
        const result = await pool.query(`SELECT provider_id, model_id,
              COALESCE(input_price_per_1k_tokens, 0)  AS input_price,
              COALESCE(output_price_per_1k_tokens, 0) AS output_price
       FROM model_profiles
       WHERE input_price_per_1k_tokens IS NOT NULL
          OR output_price_per_1k_tokens IS NOT NULL`);
