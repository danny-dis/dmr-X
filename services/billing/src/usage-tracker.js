...
export class UsageTracker {
    static RT_PREFIX = 'usage:rt:';
    static DAILY_PREFIX = 'usage:daily:';
    static MONTHLY_PREFIX = 'usage:monthly:';
    ...
    async record(record) {
        ...
        const result = await pool.query(`INSERT INTO usage_records
    ...
    async queryRecords(query) {
        ...
        const result = await pool.query(`SELECT id, tenant_id, provider_id, model_id, input_tokens, output_tokens, total_tokens, cost_cents, request_id, created_at
       FROM usage_records
       ${whereClause}
     ...
     */
    async aggregateByDimensions(tenantId, from, to) {
        const pool = getPool();
       ...
       ORDER BY total_cost_cents DESC`, [tenantId, from, to]);
     ...
     */
    async aggregateFromPostgres(tenantId, from, to) {
        const pool = getPool();
        const result = await pool.query(`SELECT
         COUNT(*)           AS total_requests,
         SUM(input_tokens)  AS total_input_tokens,
       ...
       WHERE tenant_id = $1
         AND created_at >= $2
         AND created_at <= $3`, [tenantId, from, to]);
        const row = result.rows[0];
     ...
     */
    async resetRealtimeCounters(tenantId) {
        const redis = getRedis();
