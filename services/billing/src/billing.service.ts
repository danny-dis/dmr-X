...
export class BillingService {
  ...
  /**
   * Check if a tenant has hit budget alert thresholds.
  ...
  ): Promise<Array<{ threshold: number; spentCents: number; percentUsed: number; message: string }>> {
    if (budgetCents <= 0) return [];

    const usage = await this.tracker.getRealtimeUsage(tenantId);
    const percentUsed = (usage.costCents / budgetCents) * 100;
    const alerts: Array<{ threshold: number; spentCents: number; percentUsed: number; message: string }> = [];
    ...
    for (const threshold of thresholds) {
      if (percentUsed >= threshold) {
        alerts.push({
          ...
          message: `Tenant ${tenantId} has used ${percentUsed.toFixed(1)}% of budget ($${this.formatCents(usage.costCents)} / $${this.formatCents(budgetCents)})`,
        });
      }
    }
