...
export async function createServer() {
  ...
  for (const row of registeredProviders) {
    ...
    if (authMethod === 'oauth' && row.oauth_access_token && adapter && baseUrl) {
      ...
      // Check if token is expired ΓÇö try to refresh
  ...
  if (!process.env.DMRX_ENCRYPTION_KEY) {
    logger.warn('DMRX_ENCRYPTION_KEY not set ΓÇö provider API keys are stored in plaintext');
  }
  ...
  // Start health checker ΓÇö delay initial run to allow all adapters (including
  ...
  // Background OAuth token refresh ΓÇö check every 5 minutes
...
// Start telemetry service (fire-and-forget ΓÇö must not crash the server)
   try {
     const telemetry = getTelemetryService();
     await telemetry.start();
     logger.info('Telemetry service started');
   } catch (err) {
     logger.warn({ err }, 'Failed to start telemetry service ΓÇö continuing without telemetry');
   }

  // CORS ΓÇö never use wildcard origin; always use explicit origins
